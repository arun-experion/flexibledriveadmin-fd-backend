...
class SyncCSVProductPrices extends Command
{
  ...
  public function handle() {
    ...
    $filepath = "/var/www/html/flexible-drive/flexibledriveadmin-fd-backend/public/bulkupload/cust.csv";
    $filepath1 = "/var/www/html/flexible-drive/flexibledriveadmin-fd-backend/public/bulkupload/pric-test.csv";
    $handle = fopen("/var/www/html/flexible-drive/flexibledriveadmin-fd-backend/public/uploads/products-price/product_customer_price.txt", "a");
      ...
      if (array_key_exists($acnt_code, $array_customer_category_mapping)) {
        ...
        if (array_key_exists($category_id, $array_category_price_mapping)) {
          ...
          foreach ($part_number_array as $part_number => $price_data) {
            ...
            // $product_data = Product::whereRaw("REPLACE(product_nr, '.', '') = ?", [$part_number])->where('company_sku', $part_number)->first();
            // print_r($product_data);
            // $product_data = Product::where('company_sku', $part_number)->value('id');
    ...
    if($failedEnteryCount > 0) {
      $path = "/var/www/html/flexible-drive/flexibledriveadmin-fd-backend/public/uploads/products-error-price";
      if (!file_exists($path)) {
        mkdir($path, 0777, true);
      }
      $errorFilePath = '/var/www/html/flexible-drive/flexibledriveadmin-fd-backend/public/uploads/products-error-stock/'.date('dmy')."_Log".".txt";
